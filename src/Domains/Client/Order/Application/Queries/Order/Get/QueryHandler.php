...
namespace Project\Domains\Client\Order\Application\Queries\Order\Get;
...
final class QueryHandler implements QueryHandlerInterface
{
    ...
    {
        return $this->repository->paginateByAuthorUuid(
            AuthorUuid::fromValue($this->authManager->client()->getUuid()),
            $query,
            StatusEnum::from($query->status)
        );
    }
}
